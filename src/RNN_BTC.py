...
def main(time_stamp, output):
    ...
    train = BTC_Close[:point_1 + time_stamp].values.reshape(-1, 1)
    valid = BTC_Close[point_1 - time_stamp:point_2 + time_stamp].values.reshape(-1, 1)
    test = BTC_Close[point_2 - time_stamp:].values.reshape(-1, 1)
    ...
    callBack = [ModelCheckpoint(f'./model/BTC_RNN(Input={time_stamp}, Output={output}).h5',
    ...
    print("BTC", time_stamp, output)
    print(f'{((end - start) / 20):.2f}')

    plt.figure(figsize=(8, 6))
    plt.title(f'BTC_RNN(Input={time_stamp}, Output={output})')
    plt.plot(history.history['loss'], label='training loss')
    plt.plot(history.history['val_loss'], label='val loss')
    plt.tight_layout()
    plt.savefig(f'./figure/BTC_RNN(Input={time_stamp}, Output={output})_loss.png')
    plt.show()

    model = load_model(f'./model/BTC_RNN(Input={time_stamp}, Output={output}).h5')
    ...
    plt.legend(loc='best')
    plt.tight_layout()
    plt.savefig(f'./figure/BTC_RNN(Input={time_stamp}, Output={output}).png')
    plt.show()
