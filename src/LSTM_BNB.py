...
# 讀取資料
BNB = pd.read_csv('ID_0_BNB.csv')

BNB_Close = BNB['Close']

time_stamp, output = 15, 1

point_1 = int(BNB.shape[0] * 0.7)
point_2 = int(BNB.shape[0] * 0.85)

# 70% train, 15% valid, 15% test
train = BNB_Close[:point_1 + time_stamp].values.reshape(-1, 1)
valid = BNB_Close[point_1 - time_stamp:point_2 + time_stamp].values.reshape(-1, 1)
test = BNB_Close[point_2 - time_stamp:].values.reshape(-1, 1)
...
callBack = [ModelCheckpoint(f'./model/BNB_LSTM(Input={time_stamp}, Output={output}).h5',
...
print("BNB", time_stamp, output)
print(f'{((end - start) / 20):.2f}')

plt.figure(figsize=(8, 6))
plt.title(f'BNB_LSTM(Input={time_stamp}, Output={output})')
plt.plot(history.history['loss'], label='training loss')
plt.plot(history.history['val_loss'], label='val loss')
plt.tight_layout()
plt.savefig(f'./figure/BNB_LSTM(Input={time_stamp}, Output={output})_loss.png')
plt.show()

model = load_model(f'./model/BNB_LSTM(Input={time_stamp}, Output={output}).h5')
...
plt.legend(loc='best')
plt.tight_layout()
plt.savefig(f'./figure/BNB_LSTM(Input={time_stamp}, Output={output}).png')
plt.show()
