...
# 讀取資料
DOGE = pd.read_csv('ID_3_DOGE.csv')

DOGE_Close = DOGE['Close']

time_stamp, output = 15, 1

point_1 = int(DOGE.shape[0] * 0.7)
point_2 = int(DOGE.shape[0] * 0.85)

# 70% train, 15% valid, 15% test
train = DOGE_Close[:point_1 + time_stamp].values.reshape(-1, 1)
valid = DOGE_Close[point_1 - time_stamp:point_2 + time_stamp].values.reshape(-1, 1)
test = DOGE_Close[point_2 - time_stamp:].values.reshape(-1, 1)
...
callBack = [ModelCheckpoint(f'./model/DOGE_LSTM(Input={time_stamp}, Output={output}).h5',
...
print("DOGE", time_stamp, output)
print(f'{((end - start) / 20):.2f}')

plt.figure(figsize=(8, 6))
plt.title(f'DOGE_LSTM(Input={time_stamp}, Output={output})')
plt.plot(history.history['loss'], label='training loss')
plt.plot(history.history['val_loss'], label='val loss')
plt.tight_layout()
plt.savefig(f'./figure/DOGE_LSTM(Input={time_stamp}, Output={output})_loss.png')
plt.show()

# %%

model = load_model(f'./model/DOGE_LSTM(Input={time_stamp}, Output={output}).h5')
...
plt.legend(loc='best')
plt.tight_layout()
plt.savefig(f'./figure/DOGE_LSTM(Input={time_stamp}, Output={output}).png')
plt.show()

print(f'MAE: {mean_absolute_error(test[time_stamp + output:], y_pred_avg)}')
print(f'RMSE: {np.sqrt(mean_squared_error(test[time_stamp + output:], y_pred_avg))}')


# %%

model = load_model(f'./model/BTC_ETH_LSTM(Input={time_stamp}, Output={output}).h5')
...
# 70% train, 15% valid, 15% test
train = DOGE_Close[:point_1]
valid = DOGE_Close[point_1:point_2]
test = DOGE_Close[point_2:]