...
def main(time_stamp, output):
    ...
    plt.title(f'BTC_LSTM(Input={time_stamp}, Output={output})')
    plt.plot(history.history['loss'], label='training loss')
    plt.plot(history.history['val_loss'], label='val loss')
    plt.tight_layout()
    plt.savefig(f'./figure/BTC_LSTM(Input={time_stamp}, Output={output})_loss.png')
    plt.show()

    model = load_model(f'./model/BTC_LSTM(Input={time_stamp}, Output={output}).h5')
    ...
    plt.legend(loc='best')
    plt.tight_layout()
    plt.savefig(f'./figure/BTC_LSTM(Input={time_stamp}, Output={output}).png')
    plt.show()
